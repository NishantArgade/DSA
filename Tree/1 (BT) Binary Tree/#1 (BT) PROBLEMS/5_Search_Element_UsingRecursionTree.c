...
int SearchEleOfTreeRecursion(struct BTNode *root,int data)
{
    if (root == NULL)
        return 0;
    ...
    int temp = 0;
    temp = SearchEleOfTreeRecursion(root->Lchild,data);

    if (temp == 0)
    {
        temp = SearchEleOfTreeRecursion(root->Rchild,data);
        return temp;
    }
...
int main()
{
   ...
   if (SearchEleOfTreeRecursion(root,data))
