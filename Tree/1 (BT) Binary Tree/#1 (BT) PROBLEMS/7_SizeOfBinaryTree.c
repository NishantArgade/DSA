...
// REcursive function:
int RE_SizeOfBtree(struct BTNode* root){  ///                  

    if (root == NULL)
        return 0;
    
    int LSize =RE_SizeOfBtree(root->Lchild);
    int RSize =RE_SizeOfBtree(root->Rchild);
    
    int sum = LSize +1+ RSize;
    return sum;
}
// Non-recursive(ITerative) function:
int IT_SizeOfBtree(struct BTNode* root){
  struct Queue q;
  CreateQueue(&q,100);
  int count = 0;
  if (isEmpty(f))
...
int main()
{
    ...
    printf("\nLevel-Order Traversal Befor Insertion is :");
    levelOrderTrav(root);

    // printf("\nRecursive : The Size of Binary Tree is : %d ",RE_SizeOfBtree(root) );
    printf("\niterative : The Size of Binary Tree is : %d ",IT_SizeOfBtree(root) );
