...
int RE_MinHeightOfTree(struct BTreeNode *root)
{
    if (root == NULL)
        return 0;

    int left_H = RE_MinHeightOfTree(root->Lchild);
    int right_H = RE_MinHeightOfTree(root->Rchild);
    int h;
    if (left_H > right_H) // Only change < sign for Min ele of Max ele function
        h = 1 + right_H;
    else
        h = 1 + left_H;
...
int IT_MinHeightOfTree(struct BTreeNode *root)
{
    struct Queue q;
    CreateQueue(&q, 100);
...
int main()
{
    ...
    printf("\nRecursive : Min Height of Tree is : %d ", RE_MinHeightOfTree(root));
    printf("\nIterative : Min Height of Tree is : %d ", IT_MinHeightOfTree(root));
