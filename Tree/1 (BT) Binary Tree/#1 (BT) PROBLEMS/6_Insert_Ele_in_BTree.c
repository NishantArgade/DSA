...
int InsertEleInTreeIteration(struct BTNode *root, int data)
{
    struct Queue q;
    CreateQueue(&q, 100);
...
int main()
{
    ...
    int data;
    printf("\nEnter Inserte Element :");
    scanf("%d", &data);

    if (InsertEleInTreeIteration(root, data))
        printf("\nSuccessfuly inserted element. ");
    else
        printf("\nUnsuccessfuly inserted element !");

    printf("\nLevel-Order Traversal AfterInsertion is :");
    levelOrderTrav(root);
    return 0;
}
