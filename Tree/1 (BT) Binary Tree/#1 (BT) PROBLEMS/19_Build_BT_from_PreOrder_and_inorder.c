...
struct BTNode *BuildTree(int Pre[], int Preind, int in[], int inStart, int inEnd)
{
    ...
    newNode->Lchild = BuildTree(Pre, Preind, in, inStart, INind - 1); //****
    newNode->Rchild = BuildTree(Pre, Preind, in, INind + 1, inEnd);   //****

    return newNode;
}
int main()
{
    //Build Tree using Pre and inorder Traversal :

    int Pre[] = {1, 2, 4, 5, 3, 6};
    int in[] = {4, 2, 5, 1, 6, 3};

    //For pre-order and in-order: start ind = 0 and end ind = n-1  .//****
    struct BTNode *newroot = BuildTree(Pre, 0, in, 0, 5);
