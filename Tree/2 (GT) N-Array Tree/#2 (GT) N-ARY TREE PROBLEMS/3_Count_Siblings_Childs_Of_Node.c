...
int SiblingCount(struct NTNode * current){
    int count = 0;
    while(current->Sibling){
        count++;
        printf("%d Sibling is : %d\n",count ,current->Sibling->data);
        current = current->Sibling;
    }
    return count;
}
int ChildCount(struct NTNode * root){
    int count = 0;
    root = root->Child;
    while(root){
        count++;
...
{ // Note : Root Node Doesn't have any Sibling !.
    Create_N_Ary_Tree();
     printf("The Total Child of node %d is %d\n",root->data,ChildCount(root));
    return 0;
}